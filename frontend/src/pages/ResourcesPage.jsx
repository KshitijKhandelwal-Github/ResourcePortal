  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <>
          <div style={{ fontSize: '13px', color: 'var(--gray)', marginBottom: '14px', fontWeight: 500 }}>
                ...
                {resources.length === 0 ? (
                  ...
                  resources.map(r => (
                    <tr key={r.employee_id} className="clickable" onClick={() => navigate(`/resources/${r.employee_id}`)}>
                      <td style={{ fontWeight: 600, color: 'var(--black)' }}>{r.employee_id}</td>
                      <td style={{ fontWeight: 500 }}>{r.name}</td>
                      <td>{r.cluster?.name || r.cluster_name || '—'}</td>
                      <td>
                        {r.primary_skill?.name || r.primary_skill_name ? (
                          <span className="skill-tag primary">{r.primary_skill?.name || r.primary_skill_name}</span>
                        ) : '—'}
                      </td>
